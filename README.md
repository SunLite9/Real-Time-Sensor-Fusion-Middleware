...
| Resource limit sensitivity | Cutting fusion's CPU limit from 750m to 100m took it from 0% miss rate to a complete stall in all 3 trials |
...
| Application layer fault recovery | 15s GPS dropout absorbed with RMSE barely moving (0.59m to 0.56m), correctly logged |

Both benchmarks now run 3 independent trials per level and report mean +/- standard deviation rather than a single sample, see [DESIGN.md](DESIGN.md) section 15.4 for why (an earlier single trial version produced a non monotonic latency curve that was impossible to distinguish from measurement noise with n=1).
